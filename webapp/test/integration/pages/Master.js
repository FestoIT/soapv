			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("SoId") === oObject.SoId;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].SoId;
										} else {
											sCurrentId = aItemsNotInTheList[0].SoId;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("SalesOrders", {
											SoId : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("SoId"),
							title: oBindingContext.getProperty("BuyerName")
						};
					}
				},
				...
				assertions : {
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("BuyerName");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all SalesOrders to be able to check for the total amount
