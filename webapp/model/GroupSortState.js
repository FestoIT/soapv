sap.ui.define([
	...
	return BaseObject.extend("com.festo.fiori.apps.so.apv.model.GroupSortState", {
		 ...
		 * @alias com.festo.fiori.apps.so.apv.model.GroupSortState
		 */
		...
		/**
		 * Sorts by BuyerName, or by NetAmount
		 *
		...
		/**
		 * Groups by NetAmount, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "NetAmount") {
				...
				this._oViewModel.setProperty("/sortBy", "NetAmount");

				aSorters.push(
					new Sorter("NetAmount", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "BuyerName");
			}
